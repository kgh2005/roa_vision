...
def get_median_depth_m(
    ...
    u, v: pixel coordinate, u=col, v=row
...
def pixel_to_cam_coords(
    ...
    """
    Return (X, Y, Z) meters in camera frame.
    Invalid depth returns (0.0, 0.0, -1.0).
    """
    if fx <= 0.0 or fy <= 0.0:
        return 0.0, 0.0, -1.0

    Z = get_median_depth_m(
        depth_m=depth_m,
        u=u,
        v=v,
        half_win=half_win,
